...
plugin.exports = class Plugin implements BookSource {
   ...
   */
  public static readonly ID: string = 'bvFVfvxIQiFBm91lA0idk';
  /**
   * 静态属性 TYPE  必填
   * 插件类型
   * 值类型:
   ...
   */
  public static readonly PLUGIN_FILE_URL: string = 'https://github.com/bg5tue/readcat-plugin/raw/refs/heads/main/dist/feibzw.js';
  /**
   * 静态属性 BASE_URL  必填
   * 插件请求目标链接
   */
  ...
  gbData: string[] = [
    ...
    '怠耽担丹单郸掸胆旦氮但惮淡诞弹蛋当挡党荡档刀捣蹈倒岛祷导到稻悼道盗德得的蹬灯登等瞪凳邓堤低滴迪敌笛狄涤翟嫡抵底地蒂第帝弟递缔颠掂滇碘点典靛垫电佃甸店惦奠淀殿碉叼雕凋***掉吊钓调跌爹碟蝶迭谍叠',
  ...
  UrlEncode(text: string): string {
    ...
    for (var i = 0; i < text.length; i++) {
      ...
      var reg = /^[\u4e00-\u9fa5\uf900-\ufa2d]$/;
      if (reg.test(tmp)) {
        for (var area = 0; area < this.gbData.length; area++) {
          var place = this.gbData[area].indexOf(tmp) + 1;
          if (place) {
            area += 16;
            str += '%' + getUCase((0xA0 + area).toString(16));
            str += '%' + getUCase((0xA0 + place).toString(16));
            break;
          }
        }
      } else {
        var vPos = '~!@#$%^&*()-_+={}[]|\\:;"\'<>,.?/`'.indexOf(tmp);
        str += vPos != -1 ? '%' + getUCase(tmp.charCodeAt(0).toString(16)) : tmp;
      }
    }
  ...
  async search(searchkey: string): Promise<SearchEntity[]> {
    // 拼接URL
    const url = `${Plugin.BASE_URL}/book/search.aspx?SearchKey=${this.UrlEncode(searchkey)}&SearchClass=1&SeaButton=`;
    // 获取页面内容
    const { body } = await this.request.get(url, { urlencode: 'GBK', charset: 'GBK' });
    // 要返回的搜索结果列表
    let result: SearchEntity[] = [];
    // 提取并遍历搜索结果列表
    this.cheerio(body, { decodeEntities: false })('div.listbox>div.book').each((index, element) => {
      // 提取列表项
      const item = this.cheerio(element)('div#CListTitle');
      // 加入到要返回要搜索结果列表
      result.push({
        bookname: item.children('a:first').text(), // 书名
        author: /\[\s(.*?)\s\|/.exec(item.text())[1], // 作者
        coverImageUrl: undefined, // 封面图片，这个站点搜索结果无封面
        latestChapterTitle: item.children('a:last').text(), //最新章节
        detailPageUrl: Plugin.BASE_URL + item.children('a:first').attr('href'), // 详情页URL
      })
    });
    // 返回搜索结果
    return result;
  ...
  async getDetail(detailPageUrl: string): Promise<DetailEntity> {
    ...
    // 提取书籍信息
    const bookInfo = this.cheerio(body, { decodeEntities: false })('div.bookinfo');
    // 章节列表
    let chapterList: Chapter[] = [];
    // 提取章节列表
    const chapters = this.cheerio(body, { decodeEntities: false })('div.chapterlist>ul>li');
    // 截取用于接章节URL的前段内容
    const prefix = detailPageUrl.substring(0, detailPageUrl.lastIndexOf('/'));
    console.log(prefix)
    // 遍历提取的章节列表
    chapters.each((index, element) => {
      // 提取章节信息
      var chapter = this.cheerio(element)('a');
      // 加入到章节列表
      chapterList.push({
        title: chapter.text(), // 章节标题
        url: `${prefix}/${chapter.attr('href')}`, // 章节URL
      });
    });
    // 提取简介，移除无用内容
    let intro = bookInfo.children('div.intro');
    intro.children('em').remove();
    intro.children('p').remove();
    // 返回书籍详情
    return {
      bookname: bookInfo.children('div.info').children('p').children('a').text(),
      author: bookInfo.children('div.info').children('p').children('span').text().match(/\/\s(.*?)$/)[1],
      coverImageUrl: Plugin.BASE_URL + bookInfo.children('div.bookimg').children('a').children('img').attr('src'),
      latestChapterTitle: bookInfo.children('div.intro').children('p:last').children('a').text(),
      intro: intro.text(), // 简介
      chapterList, // 与上方定义的章节列表同名，可以简写
    }
  }
  ...
  async getTextContent(chapter: Chapter): Promise<string[]> {
    ...
    // 要返回的段落列表
    let result: string[] = [];
    const count = this.cheerio(body)('div#content>p').length;
    // 提取章节段落
    const content = this.cheerio(body)(`div#content>p:gt(1):lt(${count - 4})`);
    // 遍历段落，跳过前后无用内容
    for (let i = 0; i < content.length; i++) {
      // 添加到段落列表
      result.push(this.cheerio(content[i])('p').text());
    }
    // 返回段落列表
    return result;
  }
